...
class QuantumPIAnneal():
    ...
    def __init__(self, model, latticetype = "2D", annealingrunseed = 1 , **kwargs):
        ...
        # Select numbers evenly in range of gamma_0 and gamma_T
        ...
        self.chain_strength = kwargs.pop('chain_strength', 1.0)
        print("Chain strength =", self.chain_strength)
    ...
    def pre_anneal(self):
        ...
        print("Final spin config after pre-annealing is: {}".format(
            self.spinVector), "\n")
    ...
    def quantum_anneal(self, confs, sched):
        ...
        print("Final spins configuration after quantum annealing is: {}".format(self.minConfig), "\n")
    ...
    def perform_tau_schedule(self):
        ...
        for sch in self.q_scheds:
            ...
            self.confs.append(self.minConfig)

        return np.array(self.Energies), np.array(confs) #2D np.array with size (len(self.q_scheds), numtrotterslices)
